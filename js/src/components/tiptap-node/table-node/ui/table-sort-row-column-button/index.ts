...
import { getTable, getTableSelectionType, getRowOrColumnCells, isCellEmpty } from "@/components/tiptap-node/table-node/lib/tiptap-table-utils";
...
export const tableSortRowColumnLabels = {
  row: {
    asc: "Sort row A-Z",
    desc: "Sort row Z-A",
  },
...
function canSortRowColumn(
  ...
  try {
    ...
    const hasContent = cellData.cells.some((cellInfo) =>
      cellInfo.node &&
...
function tableSortRowColumn(
  ...
  try {
    ...
    const allItems = cellData.cells.map((cellInfo, originalIndex) => {
