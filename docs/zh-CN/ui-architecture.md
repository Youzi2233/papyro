...
| App chrome | `AppShell`、`Workbench`、`MainColumn`、`ResizeRail`、`Sidebar`、`TreeSortControl`、`FileTree`、`AppHeader`、`StatusBar`、`DesktopLayout`、`MobileLayout` | 桌面和移动端 shell 已共享 `AppShell` 与 `Workbench`，侧边栏拖拽调整宽度入口已使用 `ResizeRail`，文件树行已使用 `TreeItem` 基础组件，workspace 根目录行已使用 `SidebarItem`，侧边栏搜索已使用 `SidebarSearchButton`，侧边栏创建和底部操作已使用 button 基础组件，桌面/移动端文件排序控件已共享 `TreeSortControl`，并且树排序控件已组合 `SegmentedControl`；剩余 tab chrome 还需要继续接入基础组件。 |
...
| `Toolbar` / `ToolbarZone` / `ResizeRail` | 部分已有 | `primitives/layout.rs` 拥有 `AppShell`、`Workbench`、`MainColumn`、`EditorToolbar`、`ToolbarZone`、`EditorToolButton`、`EditorTabScrollButton`、`ResizeRail` 和 `ScrollContainer`，覆盖共享 shell、设置内容滚动区、可调整 rail 与编辑器 chrome 分区；split panes 和更多滚动容器还需要继续接入。 |
