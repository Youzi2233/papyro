...
- [ ] 设置改为独立桌面窗口。
- [x] 设置窗口切换分区时大小稳定。
- [ ] 用 Papyro 设计系统组件替换原生感强的 `select`、modal、message、menu、tooltip。
- [ ] 建立 `Button`、`IconButton`、`Select`、`SegmentedControl`、`Modal/Dialog`、`Message/Toast`、`ContextMenu`、`Tooltip`、`Tabs`、`FormField` 等基础组件。
- [ ] 参考成熟开源组件系统的行为和可访问性，例如 [Radix Primitives](https://github.com/radix-ui/primitives)；参考 [shadcn/ui](https://github.com/shadcn-ui/ui) 的二次封装思路，但不直接引入 React 依赖。
- [ ] 桌面体验稳定后再做移动端布局 pass。
...
- [ ] 继续修补 Hybrid 前先调研行业通用编辑器架构：CodeMirror decorations/widgets、ProseMirror/Tiptap node views、Lexical decorators、Slate void/inline nodes、Typora 类源码/渲染切换策略。
...
- [ ] 表格编辑要文档化：新增行列、单元格导航、对齐、避免布局跳动。
- [ ] 数学公式要一等支持：inline/display 插入、实时预览、错误反馈。
...
- [ ] 替换奇怪的字体预设，改成系统优先的实用预设：UI Sans、System Serif、Reading Serif、Mono Code、CJK-friendly fallback stacks。
- [ ] 字体设置要面向普通用户：有预览文本、清晰标签、安全默认值，不把冷门字体名放在首选。
- [ ] 增加主题和 Markdown 样式 smoke/snapshot 检查，避免后续 CSS 改坏对比度、间距和代码可读性。
