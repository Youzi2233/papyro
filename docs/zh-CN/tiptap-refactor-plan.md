...
| 维度 | 当前状态 | 目标状态 |
|------|---------|---------|
| 语言 | 混合 TypeScript，且截至 2026-05-13 Markdown manager 迁移后，`js/src/` 下仍有 1 个被跟踪的 `.js` 文件且不再有被跟踪的 `.jsx` 文件 | TypeScript (.ts/.tsx) |
...
- JavaScript 存量：Markdown manager 迁移后，`js/src/` 下仍有 1 个被跟踪的 `.js` 文件且不再有被跟踪的 `.jsx` 文件。它是源码，不是生成物。剩余文件负责 Papyro 特有代码块行为；当官方 TS/TSX 组件完全接管对应行为时应直接删除。
...
- [ ] 将 `js/src/` 下剩余 1 个 `.js` 文件迁移为 `.ts`/`.tsx`，并保持无被跟踪 `.jsx` 文件；若官方 TS/TSX 组件已经接管对应行为，则直接删除旧文件
