...
| 维度 | 当前状态 | 目标状态 |
|------|---------|---------|
| 语言 | 混合 TypeScript，且截至 2026-05-13 React 支撑边界迁移后，`js/src/` 下仍有 27 个被跟踪的 `.js` 文件且不再有被跟踪的 `.jsx` 文件 | TypeScript (.ts/.tsx) |
...
- JavaScript 存量：React 支撑边界迁移后，`js/src/` 下仍有 27 个被跟踪的 `.js` 文件且不再有被跟踪的 `.jsx` 文件。它们是源码，不是生成物。剩余文件主要是需要行为覆盖后类型化的 Papyro 特有 Markdown/媒体适配，以及仍暴露旧 JS 边界的编辑器交互 helper；当官方 TS/TSX 组件完全接管对应行为时应直接删除。
...
- [ ] 将 `js/src/` 下剩余 27 个 `.js` 文件迁移为 `.ts`/`.tsx`，并保持无被跟踪 `.jsx` 文件；若官方 TS/TSX 组件已经接管对应行为，则直接删除旧文件
