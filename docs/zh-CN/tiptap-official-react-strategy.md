...
当前 `js/package.json` 里的 Tiptap 依赖都固定在同一个 `3.23.1` 版本，符合 Tiptap 包版本一致性的要求，也匹配当前 Tiptap UI CLI 组件输出。
...
新增官方 UI 表面时，不要把一长串组件直接安装进正在运行的组件树。CLI 可能提示覆盖 Papyro 已有 primitive，也可能带入 AI、协作、mention 或 emoji helper，而这些默认不属于本地优先运行时。后续要按单个表面生成、审查 diff，只合并符合产品范围的文件。
