...
| 恢复 | `components/recovery.rs` | 草稿行和空态已使用共享基础组件；compare panel 仍依赖临时 inline layout。 | 使用 `ComparePanel` 和冲突/错误状态 primitive。 |
...
- 恢复支持比较、恢复、丢弃。
- 恢复空态使用 `InlineAlert`，草稿行现在使用 `ResultRow`。
- 恢复比较能展示磁盘和草稿状态。

差距：

- 回收站仍借用了 command-modal 布局，但它其实是数据安全界面。
- 恢复 compare panel 仍使用 inline layout 字符串。
- 破坏性动作需要更强的确认和视觉层级规则。
