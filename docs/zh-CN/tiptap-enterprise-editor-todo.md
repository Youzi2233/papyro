...
- [ ] 默认隐藏可视句柄。只有在第一行或第一列附近有明确 hover 意图时展示行/列句柄。
  ...
  - 当前打磨：hover 行/列会给对应单元格添加明确 class，用整行或整列的连续主题边缘表达当前轴向，而不只依赖浮动 overlay。
...
- [ ] 整个单元格表面都能进入编辑和聚焦，不应该只有中间一小块能触发。
  - 当前覆盖：所有非交互型单元格表面，包括已有文字，现在都会先预览再提交单元格对象选区，不再落回原生文本选择。
  - 当前打磨：表格范围拖选可以从已有文字、空白单元格表面或空段落开始，跨单元格拖动会产生预期的矩形表格选区。
  - 当前打磨：短点击会在 pointer-up 时提交单个单元格 `CellSelection`，并吞掉后续原生 click，避免 ProseMirror 或浏览器立刻把对象选区替换成文本光标。
  - 当前打磨：双击单元格是明确的文字编辑入口；它会按指针位置解析 ProseMirror 文本位置并聚焦单元格内容，同时不破坏链接、按钮、输入框等交互型行内控件。
