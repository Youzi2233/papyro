...
- [x] Table：从 pipe table 解析到文档表格，支持 slash/`+` 表格尺寸选择、整表/行/列边缘选择 handle、pointer-first 工具条动作、键盘可访问的工具条导航、新增/删除行列、表格边缘快捷添加、表头切换、归一化的单元格左/中/右对齐状态、基于 `editor.can()` 的 disabled 状态、合并/拆分单元格、单元格导航、表格修复和删除。
