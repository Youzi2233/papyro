...
- [~] 为模式切换、关闭 Tab、切换 Tab、折叠侧栏、拖拽侧栏、打开设置、打开命令面板建立耗时日志。已覆盖模式切换、关闭/切换 Tab、折叠侧栏；拖拽侧栏和弹层打开待补齐。
- [~] 记录 Dioxus render prep、CodeMirror command、preview render、outline extract 的耗时和触发原因。已覆盖 editor pane、CodeMirror command、preview；outline extract 待补齐。
- [~] 建立可重复的手工性能脚本：空文档、10KB、100KB、1MB 文档各跑一遍。`docs/performance-budget.md` 已补充手工场景，后续需要固化成脚本。
- [ ] 定义体验预算：普通 chrome 操作目标低于 50ms，模式切换目标低于 100ms，输入延迟不能被 preview/outline 阻塞。
- [ ] `scripts/check.sh` 增加性能日志开关说明，避免性能回归只能靠体感。

#### 5.0.2 渲染边界收敛

- [ ] 拆分 `AppViewModel`：从一个全量 memo 改为 workspace、chrome、editor、document 分别派生。
- [ ] 让 `DesktopLayout` 只订阅 chrome 必要状态，不直接导致 `EditorPane` 重跑文档派生逻辑。
- [ ] 让 `Sidebar`、`Header`、`StatusBar` 不读取大 view model。
- [ ] 让 `EditorPane` 只接收 active editor 所需状态，避免侧栏/设置变更触发 host pool 大面积重算。
- [ ] 为 view model 派生函数补测试，确保无关状态变化不会改变对应 view model。

#### 5.0.3 文档派生数据缓存

- [ ] 为 tab content 引入 `content_revision` 或等价版本号。
- [ ] Preview HTML、outline、stats 以 `tab_id + revision` 缓存。
- [ ] Preview 和 outline 只为 active tab 计算；隐藏状态下不计算。
- [ ] 大文档 preview、outline、syntax highlight 分级降级，降级策略写入测试。
- [ ] 将同步重计算从 render path 中移出，必要时改为 effect/resource。

#### 5.0.4 编辑器宿主生命周期重设计

- [ ] 评估当前多 host / retired host 策略对关闭 Tab、模式切换、布局刷新的影响。
- [ ] 决定 active-only、active + warm spare、或 bounded host pool 策略。
- [ ] `SetViewMode` 只发送给需要变化的 host，并做重复命令去重。
- [ ] `RefreshLayout` 只在尺寸真实变化、host 可见且 runtime ready 时发送。
- [ ] Tab close 的 heavy cleanup 延迟到 idle 或批处理，避免卡住 UI。
- [ ] 为 host create/destroy/recycle 增加 JS 和 Rust 两侧 contract test。

#### 5.0.5 UI/UX 重新定标

- [ ] 定义新的桌面 shell：写作区为第一主角，侧栏、outline、工具栏、状态信息都降级为辅助层。
- [ ] 默认隐藏 outline，改为命令面板或快捷键按需展开。
- [ ] 格式工具栏从常驻 tabbar 移出，改为轻量浮层、命令面板或选择态工具。
- [ ] 重新设计 Source / Hybrid / Preview 三种模式的视觉关系，让它们共享同一套文档排版尺度。
- [ ] 重写编辑器和 preview typography：标题、段落、引用、代码、表格、列表、图片必须像同一套出版系统。
- [ ] 移除主界面中过度 card 化、过重阴影、暖纸装饰和非必要背景效果。
...
- [ ] 建立编辑器排版 token，并让 CodeMirror、Hybrid decoration、Preview 共用。
