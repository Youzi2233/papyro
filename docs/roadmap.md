...
crates/app/src/runtime.rs             app runtime、context 注入、command 组装、watcher
crates/app/src/workspace_flow.rs      workspace 与文件操作的纯流程
...
crates/ui/src/components/editor/mod.rs CodeMirror bridge、tabbar、toolbar、preview、autosave
...
- [ ] 更新 `README.md` 的编码与展示，确保中文在常见终端和编辑器中正常显示。
- [ ] 在 README 中明确“先读 roadmap，再按阶段开发”。
- [ ] 将 `docs/architecture.md` 与本文中的目标架构保持一致。
- [ ] 标记旧文档中的历史内容，避免新成员把旧设计当成现状。

### 0.2 测试基线

- [x] 确认 `cargo test --workspace` 通过。
- [ ] 增加 `cargo check --workspace --all-features` 为固定检查命令。
- [ ] 增加 `cargo clippy --workspace --all-targets --all-features` 检查。
- [ ] 增加 `cargo fmt --check` 检查。
- [ ] 增加 `npm run build` 检查 editor bundle 是否可构建。
- [ ] 增加 bundle sync check，验证 `assets/editor.js`、`apps/desktop/assets/editor.js`、`apps/mobile/assets/editor.js` 一致。

### 0.3 架构基线

- [ ] 增加 workspace dependency check，禁止错误依赖方向。
- [ ] 增加文件行数报告，长期跟踪大文件。
- [ ] 把 `workspace_flow.rs`、`runtime.rs`、`editor/mod.rs` 标为拆分优先级最高的模块。
- [ ] 为所有新增模块建立 owner 注释或文档说明。
...
- [ ] 新建 `crates/app/src/state.rs`，集中创建 runtime state。
- [ ] 新建 `crates/app/src/actions.rs`，定义 `AppAction`。
- [ ] 新建 `crates/app/src/dispatcher.rs`，把 UI action 分发到 use case。
- [ ] 新建 `crates/app/src/effects.rs`，管理 watcher、autosave、background task。
- [ ] 新建 `crates/app/src/export.rs`，迁出 HTML export 逻辑。
- [ ] 保留 `runtime.rs` 为 composition root，不再承载具体业务分支。

### 1.2 从 EventHandler 列表迁移到 action facade

- [ ] 保留 `AppCommands` 作为兼容层。
- [ ] 新增 `AppActions` 或 `AppDispatcher`，提供 `dispatch(AppAction)`。
- [ ] 将 `open_workspace`、`create_note`、`save_tab` 等命令映射为 action。
- [ ] 给 action 定义明确 payload 类型，避免到处传裸 `String`。
- [ ] 为 action dispatcher 增加单元测试。

### 1.3 收敛 UI 可见状态

- [ ] 新建 `AppViewModel`，只暴露 UI 当前需要渲染的数据。
- [ ] 将 `file_state` 派生成 `WorkspaceViewModel`。
- [ ] 将 `editor_tabs` / `tab_contents` 派生成 `EditorViewModel`。
- [ ] 将 settings 派生成 `SettingsViewModel`。
- [ ] UI 优先读 view model，通过 action 修改状态。
- [ ] 保留 raw signal 兼容窗口，逐步迁移。

### 1.4 拆分 workspace flow

- [ ] 将 `workspace_flow.rs` 拆为 `workspace/create.rs`、`workspace/open.rs`、`workspace/save.rs`、`workspace/rename.rs`、`workspace/delete.rs`。
- [ ] 将测试替身迁入 `tests/support` 或 `#[cfg(test)] mod support`。
- [ ] 把路径命名、选择目录、tree 查找等纯函数放入独立模块。
- [ ] 给每个 use case 补充失败路径测试。
- [ ] 明确“删除”是否直接删除还是进入回收站，当前阶段至少要有确认策略。
...
- [ ] `components/editor/mod.rs` 只保留模块导出。
- [ ] 新建 `pane.rs`，负责编辑器整体布局。
- [ ] 新建 `tabbar.rs`，负责 tab 渲染和关闭交互。
- [ ] 新建 `toolbar.rs`，负责格式化按钮。
- [ ] 新建 `preview.rs`，负责 HTML preview。
- [ ] 新建 `host.rs`，负责单个 editor host 生命周期。
- [ ] 新建 `bridge.rs`，负责 Rust/JS 消息协议。
- [ ] 新建 `autosave.rs`，负责 debounce 与 save request。
- [ ] 新建 `fallback.rs`，负责 runtime loading/error UI。

### 2.2 固化 Rust/JS 编辑器协议

- [ ] 在 `crates/editor` 中定义 `EditorEvent` 和 `EditorCommand`，避免 UI 内部私有定义协议。
- [ ] 使用 `serde` schema 文档化 message 格式。
- [ ] 在 `docs/editor-protocol.md` 写清 command/event 列表。
- [ ] 给 Rust 侧序列化/反序列化增加测试。
- [ ] 给 JS 侧增加 lightweight test runner。
- [ ] 测试 `set_content` 不回声触发 content_changed。
- [ ] 测试 `apply_format` 对 selection、empty selection 的行为。
- [ ] 测试 tab recycle 不串内容。

### 2.3 改善 autosave 可靠性

- [ ] 将 autosave 从 UI 组件中迁入 app effect 或 editor service。
- [ ] 每个 tab 使用 revision/version 防止旧任务覆盖新内容。
- [ ] 增加“保存中”“已保存”“保存失败”状态。
- [ ] 失败后保留 dirty，不误标 clean。
- [ ] app 退出、切换 workspace、关闭 tab 前触发 flush。
- [ ] 增加保存失败的单元测试。

### 2.4 编辑器性能预算

- [ ] 记录打开文件、切换 tab、输入、预览渲染的耗时指标。
- [ ] 设定 100KB、1MB、5MB markdown 文件的性能基准。
- [ ] 大文件禁用昂贵实时特性，显示降级提示。
- [ ] 避免每次 render 克隆完整文档内容。
- [ ] 预览渲染使用 memo/resource，避免无关状态触发重渲染。
...
- [ ] 定义三种模式：`Source`、`Hybrid`、`Preview`。
- [ ] 默认进入 `Hybrid`，保留 source mode 作为开发与高级用户模式。
- [ ] `Preview` 作为只读阅读模式，不作为主要写作模式。
- [ ] 将 `ViewMode` 从二值扩展为稳定 enum。
- [ ] 为模式切换增加状态持久化。

### 3.2 Hybrid 渲染策略

- [ ] 基于 CodeMirror extension / decoration 实现非焦点区域渲染。
- [ ] 当前光标所在 block 显示原始 Markdown。
- [ ] 非焦点 heading 渲染为视觉标题。
- [ ] 非焦点 emphasis、strong、inline code 渲染为近似成品样式。
- [ ] 非焦点 link 显示文本，但保留可编辑入口。
- [ ] 非焦点 image 显示图片预览。
- [ ] 非焦点 task list 显示 checkbox。
- [ ] 保留纯文本 source 作为 fallback。
