...
- `cargo test -p papyro-storage migrations` 覆盖 migration idempotency、已有数据库升级、schema contract、关键索引/列和 foreign key 开关。
...
- [ ] `storage`：文件系统、SQLite、watcher、migration。SQLite migration schema contract 和 watcher event mapping 已覆盖；文件系统错误路径仍需继续补齐。
...
- [x] UI a11y seam：tab close/status aria label 单元测试。
- [x] UI a11y static gate：RSX `aria-label` 属性拼写检查。
