...
- [x] 文字对比度满足长时间阅读。
- [x] 所有图标按钮有 tooltip / aria label。
- [x] 焦点环清晰，键盘导航不丢焦。
- [x] 不使用依赖文化偏好过强的视觉风格。
- [x] 常见屏幕尺寸下不出现文字溢出、遮挡、错位。
- [x] Windows 默认字体和 macOS 默认字体都要看起来正常。
...
- [x] JS decoration 只消费必要的 block hints，不成为文档真相来源。
- [x] 当前块、邻近块、远端块使用不同 decoration 等级。
- [x] 大文档中只对 viewport 附近 block 做重 decoration。
...
- [x] selection 跨 block 时不强行重排或隐藏源语法。
- [x] 失焦块的渲染不能改变文档实际内容。
...
- [x] 中文输入法连续输入、选词、回车、标点不丢字、不重复。
- [x] 输入 `# ` 后标题视觉变化不打断输入。
- [x] 粘贴图片后能插入相对 Markdown 链接并预览。
- [x] 在列表中回车、缩进、退格符合用户预期。
- [x] 代码块中输入不触发错误 decoration。
- [x] 表格编辑至少不破坏文本结构，后续再增强表格 UI。
...
- [x] 先完成已运行实例外部打开事件路由，再做系统双击聚焦已有窗口。`tao::Event::Opened` 已经通过 request channel 复用 `OpenMarkdownTarget`；完整单实例转发和窗口聚焦仍留在系统集成阶段。
...
- [ ] 启动时读取 `note_open_mode`，并注入 `ProcessRuntime` effective mode。`ProcessRuntimeSession` 已记录 configured/effective mode；在窗口体完成前 effective mode 明确保持 `Tabs`。
- [ ] 为默认值、保存、重启后生效、当前运行不变补测试。默认值、settings 持久化重启读回、当前 runtime effective mode 保持 Tabs 已有测试；真正 MultiWindow 生效测试待窗口体实现。
...
- [x] 规划 `.md` / `.markdown` 文件关联，不把完整打包发布提前塞进当前阶段。见 [file-association-plan.md](file-association-plan.md)。
- [x] 平台层提供必要文件打开事件抽象。`papyro-platform::desktop::file_paths_from_opened_urls` 负责把 desktop open URLs 收敛为路径，app 层继续做 Markdown 过滤和 use case 路由。
- [x] 系统双击只提交 `OpenMarkdownTarget { path }`，不指定 tab 或 window。当前启动参数、open event 和 request channel 已满足路径-only 约束；完整文件关联和单实例转发仍待实现。
...
- [ ] 外部删除已打开文件后的另存或关闭专用动作。Command Palette 已提供通用 `Close active note`，缺失文件专用 save as / close 状态仍待建模。
...
- [x] IME composition safety。`markdown input commands yield during IME composition` 和 `markdown input commands yield while IME composition is starting` 已覆盖 Space、Enter、Tab/Shift-Tab 和 Backspace 输入命令让路。
...
- [x] 人工 UI smoke checklist：见 [ui-smoke-checklist.md](ui-smoke-checklist.md)，后续自动化应覆盖同一组场景。
