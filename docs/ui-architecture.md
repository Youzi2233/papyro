...
| Editor | `EditorPane`, `EditorChrome`, `EditorTabButton`, `OutlinePane`, `PreviewPane`, `EditorHost`, `FallbackEditor` | Editor chrome now composes `SegmentedControl` for view mode switching. It still needs stable tab overflow rules, outline behavior, and shared Markdown visual tokens. |
