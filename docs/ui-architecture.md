...
| App chrome | `AppShell`, `Workbench`, `MainColumn`, `Sidebar`, `TreeSortControl`, `FileTree`, `AppHeader`, `StatusBar`, `DesktopLayout`, `MobileLayout` | Desktop and mobile shells now share `AppShell` and `Workbench`, file-tree rows use `TreeItem` primitives, workspace root rows use `SidebarItem`, desktop/mobile file sort controls share `TreeSortControl`, and the tree sort control composes `SegmentedControl`; sidebar footer rows still need broader primitive coverage. |
