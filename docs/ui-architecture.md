...
| Markdown editor runtime behavior | `js/src/tiptap-*.js`, `js/src/tiptap-react/`, `js/src/editor-host-runtime.js`, `js/src/editor-runtime-bootstrap.ts` | Use only when Tiptap, editor hit-testing, Markdown sync, React editor chrome, or JS host lifecycle owns the behavior. Keep Rust as the source of app state truth. |
