...
| Editor | `EditorPane`, `EditorChrome`, `EditorTabButton`, `OutlinePane`, `PreviewPane`, `EditorHost`, `FallbackEditor` | Editor chrome now composes `SegmentedControl` for view mode switching, `EditorTabScrollButton` for tab overflow controls, and `OutlineItemButton` for outline navigation rows. It still needs shared Markdown visual tokens. |
