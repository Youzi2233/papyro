...
| App chrome | `Sidebar`, `TreeSortControl`, `FileTree`, `AppHeader`, `StatusBar`, `DesktopLayout`, `MobileLayout` | File-tree rows use `TreeItem` primitives, workspace root rows use `SidebarItem`, and desktop/mobile file sort controls share `TreeSortControl`; sidebar footer rows still need broader primitive coverage. |
