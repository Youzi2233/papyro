...
Preview 和 Outline 当前都不再在 component effect 中同步执行重活：

- `OutlinePane` 只启动派生任务，实际 `extract_outline` 进入 `spawn_blocking`。
- `PreviewPane` 只启动渲染任务，实际 HTML render 进入 `spawn_blocking`。
- 两者都用 `tab_id + revision` 判断结果是否仍然属于当前文档。

这已经消除了关闭 tab、切换 active tab、进入 Preview 或打开 outline 时的同步派生放大效应。剩余风险是任务数量和执行预算：如果用户快速切换大文档，后台 blocking 任务仍可能堆积，所以后续需要统一派生任务队列、并发上限和可观测指标。
...
Outline 和 Preview 已落地同一模型：组件 effect 只启动派生任务，实际重计算进入
`spawn_blocking`。结果回到 UI 后按 `tab_id + revision` 写入缓存；如果当前 revision
已经变化，旧结果直接丢弃。Preview 在任务未完成时显示 Pending，占位；失败或超过
2 秒时显示 Failed 占位，避免渲染异常阻塞编辑 surface。
...
- 新 revision 到达时丢弃旧结果。
- Preview 失败或超时只影响 preview 面板，不阻塞编辑器输入。

剩余建议：

- 将 preview、outline、stats 的任务并发和预算统一到明确的 Document lane。
- 增加 trace 字段，记录派生任务的 tab id、revision、byte len、耗时和丢弃原因。
