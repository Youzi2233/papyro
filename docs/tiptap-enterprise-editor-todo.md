...
- Already integrated: licensed `table-node` visible table chrome, official table handle extension boundary, `@tiptap/extension-drag-handle-react`, `@tiptap/extension-node-range`, official color/highlight package paths, the React island slot model, `slash-dropdown-menu`, and `suggestion-menu`.
- Next official candidates for local Markdown UX: `slash-command-trigger-button`, `floating-element`, `drag-context-menu`, `turn-into-dropdown`, `move-node-button`, `delete-node-button`, `duplicate-button`, `reset-all-formatting-button`, `link-popover`, `heading-dropdown-menu`, `list-dropdown-menu`, `mark-button`, `text-align-button`, `undo-redo-button`, `image-node-pro`, `image-align-button`, and `image-upload-button`.
