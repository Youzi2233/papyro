...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 5 tracked `.js` files and no tracked `.jsx` files still under `js/src/` as of the 2026-05-13 math extension migration | TypeScript (.ts/.tsx) |
...
- JavaScript inventory: `js/src/` still contains 5 tracked `.js` files and no tracked `.jsx` files after the math extension migration. They are source files, not generated output. The remaining files are Papyro-specific Markdown/editor adapters and legacy editor interaction helpers that should be typed after behavior coverage or deleted when official TS/TSX components own the behavior.
...
- [ ] Migrate the remaining 5 `.js` files and no `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
