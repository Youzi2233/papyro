...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 6 tracked `.js` files and no tracked `.jsx` files still under `js/src/` as of the 2026-05-13 Mermaid renderer migration | TypeScript (.ts/.tsx) |
...
- [ ] Migrate the remaining 6 `.js` files and no `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
