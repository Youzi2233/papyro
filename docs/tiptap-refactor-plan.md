...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 38 tracked `.js` files and 3 tracked `.jsx` files still under `js/src/` as of 2026-05-13 | TypeScript (.ts/.tsx) |
...
- [ ] Migrate the remaining 38 `.js` files and 3 `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
...
- [ ] Convert `editor-core.js`, `markdown-sync-controller.js`, and `editor-host-runtime.js`
