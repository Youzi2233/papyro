...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 37 tracked `.js` files and 3 tracked `.jsx` files still under `js/src/` as of 2026-05-13 | TypeScript (.ts/.tsx) |
...
- JavaScript inventory: `js/src/` still contains 37 tracked `.js` files and 3 tracked `.jsx` files. They are source files, not generated output. They fall into three buckets: core runtime modules that must become TypeScript, Papyro-specific Markdown/media adapters that should be typed after behavior coverage, and legacy React support files that should either migrate to TSX or be deleted once the official component path fully owns the UX.
...
- [ ] Migrate the remaining 37 `.js` files and 3 `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
...
- [ ] Convert `editor-core.js` and `editor-host-runtime.js`
