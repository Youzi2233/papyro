...
| Running-instance system open | `MarkdownOpenRequest` queued by the desktop host from `tao::Event::Opened` file URLs, or by future single-instance forwarding. | Runtime receives the request channel, maps each path to `OpenMarkdownTarget`, and runs `OpenMarkdown` sequentially. The host submits only paths, not tab/window internals. | Same as `OpenMarkdown`. Empty or non-Markdown-only requests are ignored before they reach the dispatcher. | Same as `OpenMarkdown`. |
| `ContentChanged` | `tab_id`, new content. | Updates document content/revision/stats freshness, marks tab dirty, schedules autosave only for normal dirty state. | Missing tab is a no-op. Autosave storage failure writes `status_message` and marks save failure; save conflict marks `SaveStatus::Conflict` and keeps memory content. | Document, Workspace recent files on save success, Feedback. |
| `PasteImage` | `PasteImageRequest { tab_id, mime_type, data }` from the editor runtime. | Writes a workspace asset, builds relative Markdown, and queues an editor runtime `InsertMarkdown` command for the same tab. | Missing workspace/tab or unsupported/invalid image data writes `status_message`; no document state is changed until the editor runtime applies the queued Markdown and emits `ContentChanged`. | Workspace assets, Editor runtime command queue, Feedback. |
| `SaveActiveNote` / `SaveTab` | Active tab id or explicit tab id. | Saves current content after storage verifies the disk content hash, marks tab saved, updates title/recent files and the latest disk hash. | Missing workspace/tab/snapshot is a no-op. Storage failure writes `status_message` and marks save failure. Save conflict writes `status_message`, keeps memory content, and marks `SaveStatus::Conflict`. | Document, Workspace recent files, Feedback. |
| `ReloadConflictedActiveNote` | Active tab in `SaveStatus::Conflict`. | Reloads the active note from disk, replaces local conflicting content only if the tab revision is unchanged, marks tab saved and refreshes recent files/hash. | Missing workspace/tab/non-conflict tab is a no-op. Storage failure writes `status_message`; stale revision leaves local content untouched. | Document, Workspace recent files, Feedback. |
| `OverwriteActiveNote` | Active tab in `SaveStatus::Conflict`. | Saves current content through the explicit overwrite storage path, bypassing the stale disk hash only for this user-selected conflict action; marks tab saved and refreshes recent files/hash. | Missing workspace/tab is a no-op. Non-conflict tabs are ignored by the app flow. Storage failure writes `status_message` and marks save failure. | Document, Workspace recent files, Feedback. |
| `SaveConflictedActiveNoteAs` | Active tab in `SaveStatus::Conflict` and a platform save path. | Saves current content to another Markdown file inside the current workspace, records metadata/recent entry, then rebinds the current tab to the new note id/path if the revision is unchanged. | Picker cancel writes `status_message`. Missing workspace/tab/non-conflict tab is a no-op. Invalid target or storage failure writes `status_message`; stale revision leaves local content untouched. | Document, Workspace recent files/metadata, Feedback. |
